...
const fs = require('fs');
const logFileName = 'C:/Users/User/human/face_detection.log';

function prepareLogFile() {
  fs.writeFile(logFileName, '', (err) => {
    if (err) throw err;
    console.log(`Log file ${logFileName} created.`);
  });
}
        ...
        .then((result) => {
          if (result && result.face && result.face.length > 0) {
            for (let i = 0; i < result.face.length; i++) {
              ...
              const logMessage = `detected face: #${i} boxScore:${face.boxScore} faceScore:${face.faceScore} age:${face.age} genderScore:${face.genderScore} gender:${face.gender} emotionScore:${emotion?.score} emotion:${emotion?.emotion} iris:${face.iris}`;
              log.data(logMessage);
              appendToLogFile(logMessage); // 로그 파일에 저장
            }
          } else {
            log.data('  Face: N/A');
            appendToLogFile('  Face: N/A'); // 얼굴이 감지되지 않은 경우도 로그에 기록
          }
          return result;
        })
